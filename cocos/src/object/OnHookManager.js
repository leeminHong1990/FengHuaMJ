...
OnHookManager.prototype.update = function(interval){
	...
	if (this.createRoomLeftTime != null) {
		this.createRoomLeftTime = this.createRoomLeftTime - interval;
		if (h1global.curUIMgr.gameroomprepare_ui && h1global.curUIMgr.gameroomprepare_ui.is_show) {
			if (Math.floor(this.createRoomLeftTime) < Math.floor(this.createRoomLeftTime + interval)){
				h1global.curUIMgr.gameroomprepare_ui.update_room_left_time(this.createRoomLeftTime);				
			}
		}
	}
};
...
OnHookManager.prototype.setCreateRoomLeftTime = function(createRoomLeftTime){
	// 单位是秒
	this.createRoomLeftTime = createRoomLeftTime;
};