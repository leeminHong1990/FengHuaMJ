...
cutil.getTileColorType = function(handTilesButKing, upTiles){
	cc.log("upTiles : ",handTilesButKing ,upTiles)
	...
	if (suitNum > 1) {
		...
		if (honorNum == 1) {
			...
			cc.log("cccccccccccccc");
			return const_val.SAME_SUIT
		}
	}else {
		if (honorNum == 1) {
			cc.log("ddddddddddddd");
			return const_val.SAME_HONOR
		}else{
			cc.log("eeeeeeeeeeeee");
			return const_val.MIX_X_SUIT
		}
	}
};
...
// 朋胡
cutil.getFriendWin = function(uptiles, handTiles, handTilesButKing, kingTilesNum, p_wind, game_mode, prevailing_wind){
	for (var i = 0 ; i < uptiles.length; i++){
	...
	for (var i = 0; i < handTilesButKing.length; i++) {
		var tile = handTilesButKing[i];
		if (game_mode == 0) {
			if (const_val.DRAGONS.indexOf(tile) >= 0 || tile == const_val.WIND_EAST || tile == p_wind) {
				return false
			}
		}else if(game_mode == 1) {
			if (const_val.DRAGONS.indexOf(tile) >= 0 || tile == prevailing_wind || tile == p_wind) {
				return false
			}
		}
	}

	var handDict = cutil.getTileNumDict(handTilesButKing);
	for (var tile in handDict) {
		var tiles = handTilesButKing.concat([]);
		if (handDict[tile] == 4) {
			return
		}else if (handDict[tile] == 3){
			...
			if (cutil.meld_only_need_num(tiles, {}) == kingTilesNum){
				return
			}
		}
	}

	if (kingTilesNum == 1 && cutil.meld_with_pair_need_num(handTiles, {}) == 0) {
		return true;
	}
	if (kingTilesNum == 2 && cutil.meld_with_pair_need_num(handTilesButKing, {}) == kingTilesNum) {
		return true
	}
