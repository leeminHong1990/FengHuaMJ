...
var GameRoomInfoUI = UIBase.extend({
	...
	initUI:function(){
		...
		var game_mode_label = this.rootUINode.getChildByName("roominfo_panel").getChildByName("roommode_panel").getChildByName("game_mode_label")
		...
		game_mode_label.ignoreContentAdaptWithSize(true);
		round_label.ignoreContentAdaptWithSize(true);
		quantity_label.ignoreContentAdaptWithSize(true);
		pay_mode_label.setString(String(player.curGameRoom.pay_mode == 0 ? "房主支付" : "AA支付"));
		game_mode_label.setString(String(player.curGameRoom.game_mode == 0 ? "冲刺麻将" : "平打麻将"));
		round_label.setString(player.curGameRoom.game_mode == 0 ? ("底分：" + String(200)) : ("局数：" + String(player.curGameRoom.maxRound)));
		quantity_label.setString("起胡台数：" + String(player.curGameRoom.win_quantity));
