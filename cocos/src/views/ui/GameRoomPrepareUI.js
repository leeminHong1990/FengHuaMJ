...
var GameRoomPrepareUI = UIBase.extend({
	...
	initUI:function(){
		...
		// var roomid_label = room_info_panel.getChildByName("roomid_label");
		// roomid_label.setString("RoomID: " + player.curGameRoom.roomID.toString());

		var share_title = '房間号【' + player.curGameRoom.roomID.toString() + '】';
		var share_desc = '我在[奉化麻将]开了房间，快来一起玩吧';
	...
	update_room_left_time: function(roomTime){
		var mintues = Math.floor(roomTime / 60);
		var seconds = Math.floor(roomTime % 60);
		if (seconds < 10 ) {
			seconds = "0" + seconds;
		}
		var time_label = this.rootUINode.getChildByName("cur_player_panel").getChildByName("time_label");
		time_label.setString("0" + mintues + ":" + seconds);
	}
});