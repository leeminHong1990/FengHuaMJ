...
var GameRoomUI = UIBase.extend({
	...
	initUI:function(){
		...
		this.init_final_operation_panel();

		if (player.curGameRoom.finalTileWaitIdx == player.serverSitNum) {
			this.show_final_operation_panel()
		}
	...
	init_curplayer_panel:function(){
		...
		if (player.curGameRoom.player_num == 3) {
			this.directionList = ["east", "south", "west"];
			...
			for(var i = 0; i < 3; i++){
				...
				direct_img.loadTexture("res/ui/GameRoomUI/" + this.directionList[(player.serverSitNum + i)%3] + ".png");
			}
		} else {
			this.directionList = ["east", "south", "west", "north"];
			for(var i = 0; i < 4; i++){
				var direct_img = this.cur_player_panel.getChildByName("direct_img" + i.toString());
				direct_img.loadTexture("res/ui/GameRoomUI/" + this.directionList[(player.serverSitNum - player.curGameRoom.dealerIdx +  i + 4)%4] + ".png");
			}
		}
	...
	init_final_operation_panel:function(){
		var player = h1global.entityManager.player()
		var final_operation_panel = this.rootUINode.getChildByName("final_operation_panel")
		var draw_btn = ccui.helper.seekWidgetByName(final_operation_panel, "draw_btn")
		function draw_btn_event(sender, eventType){
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				player.decideFinalTile(true)
				final_operation_panel.setVisible(false)
			}
		}
		draw_btn.addTouchEventListener(draw_btn_event)

		var give_up_btn = ccui.helper.seekWidgetByName(final_operation_panel, "give_up_btn")
		function give_up_btn_event(sender, eventType){
			if (eventType == ccui.Widget.TOUCH_ENDED) {
				player.decideFinalTile(false)
				final_operation_panel.setVisible(false)
			}
		}
		give_up_btn.addTouchEventListener(give_up_btn_event)
	},

	show_final_operation_panel:function(){
		this.rootUINode.getChildByName("final_operation_panel").setVisible(true)
	},

	hide_final_operation_panel:function(){
		this.rootUINode.getChildByName("final_operation_panel").setVisible(false)
	},

	show_final_tiles_panel:function(tile_list){
		var final_tiles_panel = this.rootUINode.getChildByName("final_tiles_panel")
		for (var i = 0; i < 7; i++) {
			var tile_img = ccui.helper.seekWidgetByName(final_tiles_panel, "tile_img" + i.toString())
			...
			if (tile_list[i]) {
				...
				mahjong_img.loadTexture("Mahjong/mahjong_big_" + tile_list[i].toString() + ".png", ccui.Widget.PLIST_TEXTURE);
				mahjong_img.setVisible(true);
				continue;
			} 
			tile_img.setVisible(false);
		}
		final_tiles_panel.setVisible(true);
	},
	...
	update_curplayer_panel:function(serverSitNum){
		...
		if (player.curGameRoom.player_num == 3) {
			for(var i = 0; i < 3; i++){
				...
				if( i == ((serverSitNum + 3 - h1global.entityManager.player().serverSitNum)%3) ){
	...
	update_player_info_panel:function(serverSitNum, playerInfo){
		...
		var start_score = 0;
		if (player.curGameRoom.game_mode == 0) {
			start_score = 200
		}else{
			start_score = 0
		}
		...
		score_label.setString((playerInfo["total_score"] == undefined ? start_score : playerInfo["total_score"]).toString());
		var dealer_img = ccui.helper.seekWidgetByName(cur_player_info_panel, "dealer_img");
			...
			self.rootUINode.runAction(cc.Repeat.create(
				cc.Sequence.create(cc.DelayTime.create(0.1),
					cc.CallFunc.create(function(){
						...
						for(var i = 0; i < player.curGameRoom.player_num; i++){
							self.update_player_hand_tiles(i, player.curGameRoom.handTilesList[i].slice(0, cur_tile_num));
							if(cur_tile_num == 15 && i == player.curGameRoom.player_num - 1 && player.serverSitNum == player.curGameRoom.curPlayerSitNum){
								self.defaultKongSelected();
							}
						}
					})
				), 13
			...
			for(var i = 0; i < player.curGameRoom.upTilesList[serverSitNum].length; i++){
				...
				if(player.curGameRoom.upTilesList[serverSitNum][i].length > 3){
					...
					if(player.curGameRoom.upTilesList[serverSitNum][i][3]){
						...
						mahjong_img.setVisible(true);
						if(player.curGameRoom.kingTiles == player.curGameRoom.upTilesList[serverSitNum][i][3]){
							var kingtilemark_img = ccui.ImageView.create("res/ui/GameRoomUI/kingtilemark.png");
							this.upTileMarksList[idx].push(kingtilemark_img);
							kingtilemark_img.setAnchorPoint(0.0, 1.0);
							kingtilemark_img.setPosition(cc.p(0, 60));
				...
				if(ops){
					var from_idx = player.server2CurSitNum(ops[0]["fromIdx"]);
		...
		if(idx == 0){
			...
			for(var i = 0; i < 45; i++){
				...
				if(player.curGameRoom.discardTilesList[serverSitNum][i] > 0){
					...
					tile_img.setVisible(true);
					if(player.curGameRoom.kingTiles == player.curGameRoom.discardTilesList[serverSitNum][i]){
						var kingtilemark_img = ccui.ImageView.create("res/ui/GameRoomUI/kingtilemark.png");
	...
	init_operation_panel:function(){
		...
		this.operation_panel.getChildByName("op_btn4").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					...
					cc.log("11111111111    ",chowTilesList, player.curGameRoom.lastDiscardTile)
		...
		this.operation_panel.getChildByName("op_btn5").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
		...
		this.operation_panel.getChildByName("op_btn6").addTouchEventListener( //杠花
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
		...
		this.operation_panel.getChildByName("op_btn8").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.curSelectTile){
						...
						if(self.operation_type){
							...
							if(player.canSelfExposedKong(player.curGameRoom.upTilesList[player.serverSitNum], self.curSelectTile.tileNum)){
								player.doOperation(const_val.OP_RISK_KONG, [self.curSelectTile.tileNum]);
								cc.log("风险杠")
								self.hide_operation_panel();
							} else {
								// 检查是否可暗杠
		...
		this.operation_panel.getChildByName("op_btn13").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
						player.confirmOperation(const_val.OP_KONG_WIN, [player.curGameRoom.lastDiscardTile]);
					} else {
		...
		this.operation_panel.getChildByName("op_btn15").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
		...
		this.operation_panel.getChildByName("op_btn16").addTouchEventListener(
			function(sender, eventType){
				if(eventType == ccui.Widget.TOUCH_ENDED){
					if(self.operation_type){
						player.confirmOperation(const_val.OP_FINAL_WIN, [player.curGameRoom.lastDiscardTile]);
					} else {
						player.doOperation(const_val.OP_FINAL_WIN, [0]);
					}
					self.hide_operation_panel();
				}
			});
		...
		if(player.curGameRoom.waitAidList.length > 0){
			...
			if (const_val.SEASON.indexOf(player.curGameRoom.lastDrawTile) >= 0 || const_val.FLOWER.indexOf(player.curGameRoom.lastDrawTile) >= 0) { //抓到的是 花牌
		...
		for(var i = 0; i < const_val.OP_NUM; i++){
			...
			if(op_btn){
				...
				if(op_idx >= 0){
					...
					if(!from_type && (i == const_val.OP_CONCEALED_KONG || i == const_val.OP_RISK_KONG)){
	...
	defaultKongSelected:function(){
		...
		var player = h1global.entityManager.player();
		var handTiles = player.curGameRoom.handTilesList[player.serverSitNum];
		var handTilesKongIdx = player.canSelfRiskKong(player.curGameRoom.upTilesList[player.serverSitNum], handTiles);
		cc.log("handTilesKongIdx" ,handTilesKongIdx);
		if(handTilesKongIdx >= 0){
			var lastHandTile = this.rootUINode.getChildByName("player_tile_panel0").getChildByName("player_hand_panel").getChildByName("tile_img_" + handTilesKongIdx.toString());
	...
	playOperationEffect:function(opId, serverSitNum, tile){
		...
		} else if(opId == const_val.OP_DRAW_WIN || opId == const_val.OP_KONG_WIN || opId == const_val.OP_GIVE_WIN){
            if((cc.sys.os == cc.sys.OS_ANDROID && cc.sys.isNative) || (cc.sys.os == cc.sys.OS_IOS && cc.sys.isNative)){
	...
	//吃碰某玩家的次数
	show_all_full_count:function(idx, count, fullType){
		cc.log("show_all_full_count ",idx, count, fullType);
		var player = h1global.entityManager.player().curGameRoom;
		cc.log(player.playerInfoList)
		var all_full_panel = this.rootUINode.getChildByName("all_full_panel");
		var full_label = all_full_panel.getChildByName("full_label");
		
		all_full_panel.setVisible(true);
		all_full_panel.runAction(cc.Sequence.create(cc.fadeIn(2), cc.DelayTime.create(0.5), cc.Spawn.create(cc.fadeOut(1))));
		if (fullType == 0) {
			full_label.setString("您把 [" + player.playerInfoList[idx]["nickname"] + "] 吃碰" + count + "次");
		}else if(fullType == 1){
			full_label.setString("您被 [" + player.playerInfoList[idx]["nickname"] + "] 吃碰" + count + "次");
		}else{
			cc.log("你他妈怎么走了这里")
		}
	}
