...
class iRoomOperation(object):
	...
	# def createRoom(self, game_round, room_mode, is_agent):
	def createRoom(self, player_num, win_quantity, game_round, king_num, pay_mode, game_mode, is_agent):
		DEBUG_MSG("create room, {0}, {1}, {2}, {3}, {4}, {5}, {6}".format(player_num, win_quantity, game_round, king_num, pay_mode, game_mode, is_agent))
		if game_round <= 0:
			game_round = 4
		...
		def callback(content):
			...
			try:
				...
				card_cost, diamond_cost = switch.calc_cost(game_round, game_mode, pay_mode, player_num)
		...
		if room:
			...
			if room.is_agent == 1:
				...
				if switch.DEBUG_BASE:
					...
					utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost ,callback, "FengHua RoomID:{}".format(self.roomID)) # reason 必须为英文
			else:
				room.reqEnterRoom(self, True)
				self.room = room
				KBEngine.globalData["GameWorld"].addRoom(room)
	...
	# s2c
	def readyForTime(self, prepare_ts):
		''' 准备阶段倒计时 '''
		if getattr(self, 'client', None):
			self.client.readyForTime(prepare_ts)
	...
	def notifyFinalTile(self):
		if getattr(self, 'client', None):
			self.client.notifyFinalTile()

	def decideFinalTile(self, isShow):
		""" 是否要最后一张牌 """
		if self.room:
			self.room.decideFinalTile(self, isShow)

	def showFinalTile(self, tile, idx):
		if getattr(self, 'client', None):
			self.client.showFinalTile(tile, idx)

	#s2c 检查吃碰的次数
	def all_full_plyer(self, idx, count, fullType):
		if getattr(self, 'client', None):
			self.client.allFullPlyerCount(idx, count, fullType)