...
def checkIsSitWind(wind_tile, upTiles, handTiles, handTilesButKing, kingTilesNum, kingTiles):
	...
	if wind_count == 3:
		...
		for j in range(len(wind_handTiles))[::-1]:
...
def checkIsWordColor(upTiles, handTiles, handTilesButKing, kingTilesNum, kingTiles):
	...
	for i in range(len(dragon_count)):
		...
		if dragon_count[i] != 0 and kingNum >= need_kingtile_num:
			for j in range(len(dragon_handTiles))[::-1]:
...
def getFriendWin(uptiles, handTiles, handTilesButKing, kingTilesNum, p_wind, game_mode, prevailing_wind):
	...
	for tile in handDict:
		...
		if handDict[tile] >= 4:
			return
		if handDict[tile] == 3:
			for i in range(len(tilesButKings))[::-1]:
				if del_num == 2:
					break
				if tilesButKings[i] == tile:
					del tilesButKings[i]
					del_num += 1
			DEBUG_MSG("aaaaaaa {0}".format(meld_only_need_num(tilesButKings, {})))
			if meld_only_need_num(tilesButKings, {}) == kingTilesNum:
				return 
	...
	for i in range(len(twoSides)):
		...
		if len(twoSides[i]) == 2:
			for j in range(len(twoSides[i])):
				for k in range(len(handTwotiles))[::-1]:
...
def getCheckWinThorw(handTiles, finalTile, kingTiles):
	...
	if meld_only_need_num(handTilesButKing, {}) <= kingTilesNum:
		return True
	return False
